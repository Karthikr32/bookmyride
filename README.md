...
**6. Enhanced Application Functionalities for Real-World Use:** Beyond core modules, **_BookMyRide_** provides several advanced capabilities that elevate the system to professional-grade:  
...
<details>
  <summary><b>📄 View Request flow</b></summary>  <br>  
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;⮟  
**Service (post-processing: Entity → DTO, `ServiceResponse` assembly)**  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;⮟  
**Controller (standardized response via `ApiResponse`)**   
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;⮟  
**Client Response**  
</details>
...
> **Note:** Frontend/client extracts the token from `data` for subsequent requests.   

**Using the JWT Token**     
- Include the token in the Authorization header of all protected API requests: **`Authorization: Bearer <JWT_TOKEN>`**.  
...
## Folder Structure   
<details>
  <summary><b>📁 Folder Structure (Click to Expand)</b></summary>   <br>  
...
<details>
  <summary><b>📄 View ER Diagram (Click to Expand)</b></summary>  <br>  
...
- **Success & Error Responses** with collapsible screenshots & JSON for quick visual inspection
...
> **Important Note:** Must Use This Dummy Credentials (as configured in application.properties) for Safe & Secure Login. 
...
| `Exception`                 | **500 Internal Server Error** | Unexpected internal error             |    
...
**5. JWT Token Generation & Successful Authentication Response**   
...
- If authentication succeeds, returns **200 OK**, a success message & JWT token in the `data` field.  
...
#### ⚠️ Edge Cases & Developer Notes  
**1. Username & Identity Management**  
- Updating the full name auto-generates a new username.
- Admins are prompted to re-login to ensure JWTs remain valid.
- Only after re-login can admin operations (password change, location management, protected resource access) be performed.

**2. Role & Access Enforcement**  
- ADMIN-only endpoints prevent unauthorized profile updates.
- Strict separation between management and regular users avoids credential overlaps.
- Invalid or expired tokens result in immediate request rejection.

**3. Field Validation & Auditing**  
- Only modified fields are updated to prevent unnecessary writes.
- `profileUpdatedAt` timestamps support auditing and tracking of profile changes.
- Gender and role values are restricted to predefined enums; invalid inputs return **400 Bad Request** with clear messages.

**4. System Integrity Guarantee**  
- Ensures JWTs are always tied to the current username.
- Prevents usage of outdated tokens after identity changes.
- Maintains overall system security and consistent account management.
</details>  
...
#### ⚙️ How the Backend Validates This 
...
#### ⚠️ Edge Cases & Developer Notes    
**1. Token-Only Attack Prevention**
- Even if JWT is stolen, old password verification blocks unauthorized password changes.
- Stale tokens for deleted or modified accounts → **404 Not Found**.

**2. Strong Password Enforcement**
- DTO-level validation prevents weak passwords (blank, short, simple patterns).
- Centralized annotations allow future extensions: regex complexity, history checks, breached password screening.

**3. Audit & Compliance**
- `passwordLastUpdatedAt` ensures tracking for security audits.
- Only updated after successful validation and password change.

**4. Granular RBAC & Access Control**
- ADMIN-only enforcement prevents privilege escalation.
- Can be extended for microservices with role hierarchies or permission matrices.

**5. Enterprise Benefits**
- **Zero Trust:** JWT + old password + role + DB verification
- **Audit-Ready:** Consistent error codes, timestamping, no sensitive leaks
- **Scalable:** Utilities reusable across endpoints
- **Production Note:** Rate limiting, MFA, and session invalidation recommended
</details>
...
#### 📤 Success Response
<details>
  <summary>View success response screenshot</summary>  <br>  
{   <br>   
&nbsp;&nbsp;&nbsp; "staus": 200,   <br>  
&nbsp;&nbsp;&nbsp; "data": {   <br>   
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; "id": 1,   <br>  
...
- Every request relies on a valid UserPrincipal extracted from the JWT.
- Protects against stale or compromised tokens: even structurally valid JWTs won’t work if the account is deleted, disabled, or modified.
- Prevents unauthorized access and ensures only active sessions can reach sensitive endpoints.

**2. Strict Role Enforcement**  
- Endpoint is ADMIN-only. Centralized role checks block privilege escalation, ensuring standard users—even with valid JWTs—cannot access admin data.
- Easily extendable for future role hierarchies or permission matrices in microservices.

**3. Data Privacy & Safe DTO Mapping**  
- Management entity is mapped to a DTO before returning.
- Sensitive fields like passwords, internal IDs, or audit metadata are never exposed.
- Maintains transparency for frontend consumption while adhering to security and privacy standards.

**4. Scalability, Reuse & Audit-Readiness**  
- Validation and mapping logic is centralized via `UserPrincipalValidationUtils` and `ManagementMapper`.
- Reduces duplicate code, ensures consistent behavior across endpoints, and allows easy extension for:
   - Additional profile fields
   - Audit logging
   - MFA hooks or advanced security features
- Supports audit-ready practices with standardized responses and controlled data exposure. 
</details>  
...
#### 📝 Description  
This API allows a **Management/Admin user** to create a new location by adding City, State, and Country in a hierarchical structure. It ensures uniqueness and prevents conflicts through **multi-level validation, enum parsing, and database constraints**.  

**Key Highlights:**  
- **Admin validation:** Confirms the authenticated admin before performing operations.
- **Complete location creation:** Allows creation of a full location set (city, state, country) via a single DTO.
- **Hierarchical consistency:** Ensures proper creation or reuse of entities while maintaining strict validation.
- **Enum parsing:** State and Country values are validated via ParsingEnumUtils.
- **Uniqueness enforcement:** Prevents duplicates in main tables (cities → states → countries) and the MasterLocation projection table.
- **Automatic sync:** Updates the MasterLocation table after successful creation.
- **Full protection:** Secured with JWT authentication, ADMIN-role enforcement, DTO validation, optimistic locking, and projection sync.
...
#### 📤 Success Response
<details> <summary>View screenshot</summary>  <br>  
...
#### ⚠️ Edge Cases & Developer Notes   
**1. Bulk Insert Strategy & Performance Considerations**  
- Designed for **large-scale location imports** (hundreds or thousands of entries).  
- Shows how to **batch without overloading DB connections** and avoid memory issues.  
- Explains trade-offs: e.g., why we process sequentially instead of using parallel threads to prevent deadlocks.  

**2. Error Reporting & Admin Feedback**  
- Each entry generates a **detailed status**: success, duplicate, or error.  
- Helps admins **quickly identify problematic entries** without manually checking the database.  
- Useful for operational efficiency in production environments.  

**3. Data Hygiene & Normalization Lessons**  
- Enforces controlled data input: trimming spaces, parsing enums, and validating known values.  
- Prevents future data inconsistencies that could break analytics or integrations.  
- Explains why strict input normalization upfront saves time and prevents silent bugs.  

**4. Partial Success Philosophy**  
- Batch inserts do **not roll back entirely** on failure; instead, they continue to process remaining entries.  
- Balances **operational resilience vs strict ACID rules** — ensures admins maximize inserted data while maintaining clarity on failures.  

**5. Scalability & Maintenance Insights**  
- Centralized utilities (`ParsingEnumUtils`, `MasterLocationService`) reduce duplicate logic.  
- Future-proofed design: easy to add new countries, states, or validation rules without touching core loops.  
- Explains **design reasoning**, not just mechanics.  

**6. Operational Edge Cases to Consider**  
- Admins may submit repeated city-state-country combos → system logs as duplicate.  
- Concurrent submissions → optimistic locking ensures one success, others skipped gracefully.  
- Midnight / timestamp edge cases in audits: e.g., massive bulk insert at day-end for reporting.
</details>  
...
Before using this API, ensure sufficient location data (**Country → State → City**) exists in the system. This endpoint demonstrates **filtering, sorting, pagination, and hierarchical responses**, which are best experienced with multiple location records. If the database has few or no entries, the API will work but won’t fully showcase its capabilities. To prepare test data, use the **Bulk Location Creation API (#6)** to insert multiple locations first.  

<details>
<summary>View Sample Location Data (for testing)</summary>  <br>  
[   <br>  
...
- Invokes `PaginationRequest.getRequestValidationForPagination()` to validate: `page >= 1`, `size >= 1`, `sortBy` in allowed fields, `state.country.name`, `createdAt`, `updatedAt` and `sortDir`.
...
> **Note:** Only single entry are shown for illustration purpose. This API returns all records in the requested page, each following the same structure with City → State → Country metadata.  
</details>   
...
- Uses `ParsingEnumUtils` to validate and convert input strings to enum types: `Country` & `State`.
...
- After city, state, and country for Main tables then system updates:  
...
**I. Management Hierarchy Tables**  
- `CountryEntity`
- `StateEntity`
- `CityEntity`  
 
**II. Global MasterLocation Repository**  
...
- Deletion requires both: `CityEntity` & `MasterLocation`.
...
- This API ensures only one admin can modify that location at any given moment. 

**3. Transactional Atomic Deletion**  
- The entire delete operation is atomic. Which guarantees:
...
And if any failure then triggers:  
...
- If any input fails parsing → Returns **400 BAD_REQUEST** with precise error details. This avoids unregulated string inputs flowing deeper into business logic.
...
**5. Transactional Atomicity, Audit Logging & Strict Referential Integrity**  
...
The system first validates the `UserPrincipal` bound to the JWT token. This ensures:  
- The JWT token is cryptographically valid, untampered, and not expired.  
- A lookup is performed against the token revocation registry to detect manually invalidated or blacklisted tokens (e.g., logout, credential rotation, detected compromise).  
- `UserPrincipalValidationUtils.validateUserPrincipal(...)` confirms that:  
...
- Inside a transactional boundary, The bus is deleted atomically & On success, an audit log entry records _Management ID, username, bus ID, timestamp_.
- This guarentees:
  - Non-repudiation
...
**3. Exception Masking & Security Hardening**  
...
#### ⚙️ Backend Processing Flow  #### ⚙️ Backend Processing Flow   
**1. Multi-Stage Request Parameter Normalization (The First Guard Rail)**  

Before any domain logic runs, the controller enforces parameter correctness. This is not limited to checking for empty strings; it performs **semantic validation** to ensure the request is meaningful and processable.  

**I. Input Coherence Validation**  
Using `RequestParamValidationUtils.listOfErrors()`, the system verifies that:  
- All required parameters (`from`, `to`, `travelDate`) are present.  
- Values are non-empty, non-null, and not just whitespace.  
- Inputs follow acceptable structural patterns so later parsing is safe.  

This protects the service layer from malformed data and ensures that expensive database queries are never attempted on invalid input.  

**II. Fail-Fast Strategy**  
- Any validation error is returned immediately.  
...
Travel date is mission-critical because time-dependent availability depends on it. Validation follows a multi-layered approach.  

**I. Format Auto-Detection**  
- Presence of `/` or `-` → interprets as human-readable formats (**dd-MM-yyyy** or **dd/MM/yyyy**).  
- Absence of symbols → interprets as HTML-standard **yyyy-MM-dd**.  

This ensures client flexibility while maintaining strict consistency.  

**II. Strict ResolverStyle Enforcement**  
The parser uses `ResolverStyle.STRICT` to reject:  
- Invalid dates (e.g., `31-02-2025`).  
- Silent rollovers where invalid values auto-correct.  
- Leap-year misinterpretations.  

Strict parsing guarantees temporal correctness, which is fundamental for reliable scheduling.  

**III. Past-Date Rejection**  
- Rejects any `travelDate` earlier than the current day.  
- Prevents searching for routes in the past and misleading user expectations.  
- Ensures availability logic operates only on future schedules.  

**3. Sorting Field Whitelisting & Query Safety Enforcement**  
- Sorting is strictly controlled to protect the database from high-cost operations and unsafe ordering. Only three fields are permitted for client-side sorting: `id`, `departureAt` & `fare`.
- Any attempt to sort by unauthorized fields (e.g., `busName`, `busType`) results in **400 BAD REQUEST**, ensuring safe and stable queries.  

**4. Master Location Validation (Route Semantics Validation)**  

The system validates that search parameters map to canonical locations in the `MasterLocation` table. This prevents invalid routes and maintains referential integrity.  
...
**II. Layered Error Behavior**  
- **Malformed format → 400 BAD REQUEST**: input structure invalid  
- **Valid format but location not found → 404 NOT FOUND**: input structurally correct but not in the dataset  

**III. Route Integrity Rule**  
- Routes where `from` equals `to` are rejected to prevent nonsensical queries.  

**5. Deterministic Multi-Filter Query Handling**  

Filters are processed through a **Deterministic Filter Combination Algorithm**, avoiding generic dynamic query builders.  

**Why Deterministic Branching?**  
- Predictable, testable execution paths  
- Zero ambiguity between filter combinations  
- Explicit branching for maintainability  
- Optimal performance using precompiled JPQL queries  
- Exactly one repository query executed per request  
...
Within each branch:  
- Regex patterns ensure filter correctness  
- Enum values are parsed safely  
- Time-range boundaries convert into validated hour segments  
- The exact repository method mapped to that combination is executed  

This guarantees a **deterministic, conflict-free search pipeline**.  

**6. Structured Repository Query Execution**  

Each valid filter combination resolves to a **dedicated repository function**, ensuring:  
- Zero ambiguity in query selection  
- High performance through index-friendly queries  
- Predictable JPQL structure  

**Examples:** `filterBusByLocationWithBothTypeAndTime()`, `filterBusByAcTypeAndTimeRange()`.  

The system avoids a single “giant dynamic query” to ensure **predictable query planning**.  

**7. DTO Transformation & Travel-Date Awareness**  

`BusMapper.busToBusUserDto()` transforms entity objects into user-facing DTOs, handling:  
- Formatting departure and arrival timestamps  
- Exposing enums as canonical client-safe strings  
- Computing seat availability based on travel date  
...
- The deterministic branching ensures no two branches ever overlap. For example: (AC + TimeRange) & (SeatType + TimeRange)
...
Booking in **_BookMyRide_** follows a strict, state-driven workflow: **Start → Edit → Continue → Confirm**   

This sequence is mandatory to ensure seat-locking consistency, pricing accuracy, and data integrity. Skipping or reordering steps will result in invalid operations.  

📘 Before using the upcoming booking APIs, readers are strongly encouraged to review the **Booking Workflow Overview**, which explains the lifecycle, state transitions, and rules governing all booking actions.
</details>  

### 📘 Booking Workflow Overview (High-Level Booking Lifecycle)  
<details> 
  <summary><strong>Start → Edit → Continue → Confirm</strong></summary>  <br>  
...
- **Deterministic Progression** — The booking process always unfolds in the same order. No step can be skipped, repeated incorrectly, or executed out of turn. This prevents incomplete or inconsistent reservations.
- **State-Based Permissions** — Each action is allowed only when the booking’s status and payment state match the required conditions. For example:  
...
- **Immediate Seat Locking** — Seats are deducted the moment a booking is started. This ensures accurate availability even during simultaneous booking attempts. Optimistic locking is used to detect and gracefully handle conflicts when multiple users select the last remaining seats.
- **Automatic Expiry Enforcement** — Every booking includes an expiration timestamp (`bookingExpiresAt`). Once expired, the booking is invalidated automatically, seat locks are released, and further operations are blocked. This prevents abandoned bookings from holding seats indefinitely.  
...
- **Booking A – Smooth booking flow** — _(Start → Edit → Continue → Confirm → Success)_  
- **Booking B – Cancellation flow** — _(Start → Edit → Cancel before confirmation)_  
- **Booking C – Auto-expiry flow** — _(Start → No action → Automatically expired after the configured duration)_
...
- `travelAt` can be **dd-MM-yyyy or dd/MM/yyyy** — strict mode parsing ensures no invalid / past dates.

> **Note:** For different workflow scenarios, you can use separate Start Booking requests with your own sample values:  
...
Before proceeding, the system checks if the provided mobile number belongs to: A **Management/ADMIN user** or any restricted user category. If yes → **403 FORBIDDEN** – **Access Denied**, preventing unauthorized users (like Admins) from booking seats meant for public users.  
...
- This step is the heart of Start Booking. The system checks:
   - If booked seats is lesser than `availableSeats` in Bus data.  
   - If insufficient: → **409 CONFLICT** – `INSUFFICIENT_SEATS`    
- Or if request is valid, then:  
...
- Meanwhile if Any optimistic lock exception is occured that turned into a user-friendly: **409 CONFLICT** – Seats just got booked by others.
...
- The system fetches the existing booking by ID, responding with **404 NOT_FOUND** if absent. It enforces edit eligibility by requiring `bookingStatus = PENDING && paymentStatus = UNPAID && bookingExpiresAt > now()`.
- This editing feature is **rejected** and returns → **403 FORBIDDEN** with contextual messages. for the below other booking statuses:
...
- If another process (background expiry job or parallel request) modifies the booking concurrently: `OptimisticLockException` or `ObjectOptimisticLockingFailureException` is thrown by the **JPA entity version field**.
- When this happens, the system returns: **408 REQUEST_TIMEOUT** with a clear message: the booking expired before edit completion. This ensures atomicity and accuracy as like real-world applications.  
...
If the booking is **not found** → `404 NOT_FOUND`. If the booking is **ineligible** → `403 FORBIDDEN` with a contextual message.  
...
| HTTP Code | Status Name           | Meaning                              | When It Occurs                                                 |
| --------- | --------------------- | ------------------------------------ | -------------------------------------------------------------- |
| **400**       | BAD_REQUEST           | Invalid booking ID                   | Booking ID is invalid or malformed                             |
| **403**       | FORBIDDEN             | Booking not eligible                 | Booking already processed, cancelled, confirmed, or expired    |
| **404**       | NOT_FOUND             | Booking not found                    | No booking data exists for the given Booking ID                |
| **408**       | REQUEST_TIMEOUT       | Booking expired or conflict detected | Booking expired before continue or optimistic locking detected |
| **500**       | INTERNAL_SERVER_ERROR | Internal server failure              | Unexpected backend error during booking continuation           |  
...
> 💡 **Note:** Only UPI or Card or QR Code or Bank Transfer, Net Banking are allowed. You can use any of these values.  
...
- Then, saved **transactionally**, and logs detailed info: "CONFIRM BOOKING: Booking ID X has been confirmed successfully for User Y-Z".   
...
| HTTP Code | Status Name           | Meaning                              | When It Occurs                                                             |
| --------- | --------------------- | ------------------------------------ | -------------------------------------------------------------------------- |
| **400**       | BAD_REQUEST           | Invalid request data                 | Invalid booking ID or unsupported payment method                           |
| **403**       | FORBIDDEN             | Booking not eligible                 | Booking is in an invalid state (PENDING, CANCELLED, EXPIRED, or CONFIRMED) |
| **404**       | NOT_FOUND             | Booking not found                    | No booking data exists for the given Booking ID                            |
| **408**       | REQUEST_TIMEOUT       | Booking expired or conflict detected | Booking expired before confirmation or optimistic locking detected         |
| **500**       | INTERNAL_SERVER_ERROR | Internal server failure              | Unexpected backend error during booking confirmation                       |  
...
- Only the first confirmation attempt succeeds; all subsequent overlapping attempts receive: **408 REQUEST_TIMEOUT** → stale or conflicting update. This prevents:
  - Double confirmation
  - Duplicate tickets
...
**I. Ticket Generation (TK)** via `public static String generateTravelTicket(Integer digits, Long bookingId)`  
...
**II. Transaction ID Generation (TNX)** via `public static String generateTransactionId(Integer digits, Long bookingId)`
...
| --------- | --------------------- | ------------------------------------ | ---------------------------------------------------------------------- |
| **400**       | BAD_REQUEST           | Invalid booking ID                   | Booking ID is invalid or malformed                                     |
| **403**       | FORBIDDEN             | Cancellation not allowed             | Booking is no longer eligible for cancellation (already confirmed)     |
| **404**       | NOT_FOUND             | Booking not found                    | No booking data exists for the given Booking ID                        |
| **408**       | REQUEST_TIMEOUT       | Booking expired or conflict detected | Booking has expired or optimistic locking detected during cancellation |
| **500**       | INTERNAL_SERVER_ERROR | Internal server failure              | Unexpected backend error during booking cancellation                   |  
...
#### 📝 Description  
This endpoint serves as the **central access point for management-level users (Admin role)** to view, audit, analyze, and filter every booking in the system. It functions as the **primary operational dashboard API**, supporting large-scale administrative workflows with precision and reliability. Designed for **high-volume environments**, the endpoint ensures **predictable, safe execution** through strict validation and **deterministic request handling**. It allows admins to efficiently review daily transactions, conduct investigations, and perform business analytics without compromising data integrity or performance. A key strength of this API is its **collision-free filter resolution strategy**, where the backend guarantees that **exactly one repository call** is executed per search pattern. This ensures **consistent query behavior and optimized performance**, even under heavy load. This endpoint enforces **strict role-based access control** — only administrative users can access it. **Passengers or regular users are completely restricted**, ensuring that sensitive operational data always remains secure.  
...
- On successful retrieval, the API returns an HTTP **200 OK** response using the `ManagementBookingDataDto`. The payload delivers a **complete administrative snapshot** of each booking, combining booking-level details, bus metadata, and passenger information into a single, dashboard-optimized structure.
- Pagination metadata is added through `ApiPageResponse`, supporting smooth table rendering and large-volume navigation.  
...
**2. With Filter: page=1, size=10, sortBy=id, sortDir=desc, keyword=Cancelled**  
<details> 
  <summary>View Full Response</summary>  <br>  
...
The response DTO exposes only operationally relevant booking, bus, and passenger attributes, intentionally excluding internal audit fields, schema identifiers, authentication metadata, and internal relational structures. This achieves:
...
> 💡 **Tip:** Replace the sample values with any valid mobile number and password. Please ensure not repeating the same number used for Management auth. My system automatically handles both new registrations and existing user logins based on the provided credentials.  
...
> **Note:** Users can update both name, email & other info after signup. This approach prevents null values in BI reports, booking summaries, and system notifications, ensuring data consistency across the platform. 
...
**4. Mobile Uniqueness & State Consistency**     
...
> 💡 **Tip:** You can replace the above values with your sign-up mobile number & password. My system will handle this gracefully.
...
| `Exception`                 | 500 Internal Error | Unexpected authentication failure      |  
...
**3. Why Profile Completion Is Strictly Enforced Before Returning Profile Data ?**   
...
> **Note:** This mirrors real-world UX patterns: `“Let the user start using the app, and collect useful data organically along the way.”`  

**4. Why Users Must Not See Dummy/Placeholder Data ?**  
...
**I. The API blocks access until profile is completed**    
...
**II. This is common in real-world apps**  
...
**5. Developer Insight: Why this decision matters long-term**  
...
**Important** — This step enforces a **defense-in-depth strategy** and aligns with industry-standard security practices, safeguarding accounts against token-only attacks and unauthorized access.   
...
**3. Security Auditing & Future-Ready Architecture**  
...
**2. Pagination & Sorting Validation**  
...
**I. Nested DTO Structure (BookingListDto)**   
...
#### ⚠️ Edge Cases & Developer Notes  
**1. Concurrent Booking Data Changes**  
- A booking could be updated or cancelled by another system process (e.g., admin adjustment) while a user fetches their history. Service must handle potential **stale data** or optimistic lock conflicts.
- If partial booking info is returned (e.g., some tickets already deleted), frontend should handle missing/nullable nested DTOs gracefully.

**2. High-Volume & Performance Scenarios**  
- Users with hundreds or thousands of bookings may stress the DB. Ensure **indexed queries, batch fetching**, and **efficient nested mapping** to avoid timeouts.
- Sorting by rarely indexed fields (like `finalCost`) can cause slow queries; developers should consider fallback sorting or query hints for large datasets.

**3. Future-Proofing & Extensibility Notes**  
- New fields (like seat layout, driver info, loyalty points) may be added to `BookingListDto`. Ensure **backward compatibility** with older frontends.
- Nested DTOs should **safely handle nulls and optional data**, so future features don’t break existing clients.
</details>   
...
#### 📝 Description  
This API acts as the **centralized passenger directory** for the management layer, providing authorized **Management users** full visibility into registered and guest passengers. It is built for dashboards, audits, behavior analysis, fraud detection, and intelligence workflows, with secure and predictable execution through strict validation and deterministic request handling. The endpoint supports passenger inspection with optional booking activity, **stable pagination, multi-field sorting,** and **prefix-based search**. Structured DTO responses prevent internal model exposure, while a collision-free filtering engine ensures each request maps to a single, unambiguous query path, maintaining consistent performance under administrative load. Access is strictly limited to Management authorities, ensuring protection of sensitive operational data.  
...
**II. Logical Parsing and Canonicalization**  
...
**4. Deterministic Repository Execution**   
...
**5. Database-Level Aggregation & JOIN Strategy**  
...
**6. Service-Layer Result Construction & Page-Oriented Semantics**  
...
#### ⚠️ Edge Cases & Developer Notes   
**1. High-Volume Query Stress**
- Users with **hundreds/thousands of bookings** may stress pagination and nested mapping.
- Developers should monitor **DB query plans, indexed fields, and DTO mapping efficiency**, or implement **streaming / batching** to avoid timeouts.

**2. Unexpected Enum or Schema Changes**  
- If AC types or bus categories are updated in DB, or new optional fields are added to DTOs (like seat layout, amenities), **old frontends must remain compatible**.
- All nested DTOs should safely handle **nulls, missing fields, or extra fields**.

**3. Partial Failures / Downstream Service Issues**  
- If external services (payment, loyalty points, notification) fail after booking retrieval, dashboards should **gracefully degrade** and show cached or partial information rather than breaking.
- Logging these incidents is important for debugging rare production failures.

**4. Data Drift & Reporting Consistency**  
- Aggregated metrics (occupancy %, total revenue) may **temporarily drift** if bookings are updated concurrently.
- Developers should consider **read-consistency guarantees**, caching, or eventual consistency notifications for high-traffic dashboards. 
</details>   
...
#### 📝 Description  
This API serves as the definitive **Management-level** reporting endpoint for the **Passenger/AppUser module**, providing per-user booking statistics within a specified date range, including total bookings, total revenue, and the most recent booking timestamp. It supports optional **gender** and **role** filters, enabling segmentation by demographic or user type. Designed for dashboards, audits, and executive oversight, the API aggregates data directly in the repository via **JPQL constructor projections** (`BookedAppUserReportDto`), ensuring high-performance, accurate, and memory-efficient computation while separating entity internals from API-facing DTOs. Full **pagination** and **multi-field sorting** support allow scalable, real-time analytics across thousands of passengers, maintaining consistent UI behavior.
...
**I. Syntactic Date Screening (Format Layer)**  
...
**II. Semantic Parsing & Canonical Normalization (Logic Layer)**  
...
Any failure triggers **400 BAD_REQUEST** with precise diagnostic details. Only fully normalized timestamps proceed to the service layer.   
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"totalBookings": 1,<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"totalRevenue": 4396.00,<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"recentBookedAt": "2025-12-25T19:47:22.500933"<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;},<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;...<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;],<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"totalPages": 1,<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"last": false<br>
&nbsp;&nbsp;&nbsp;&nbsp;},<br>
&nbsp;&nbsp;&nbsp;&nbsp;"message": "Users data found for the given dates in this page 1"<br>
}
</details>   

**2. With Filter: page=1, size=10, sortBy=totalBookings, sortDir=desc, startDate=20/12/2025, endDate=27/12/2025, gender=female**
<details> 
  <summary>View screenshot</summary>  <br>  
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"appUserId": 6,<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"name": "Billie Elish",<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"email": "elish@gmail.com",<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"mobile": "8XXXXXXXXX",<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"gender": "Female",<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"role": "Guest",<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"totalBookings": 1,<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"totalRevenue": 659.00,<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"recentBookedAt": "2025-12-26T13:39:11.804044"<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;],<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"totalPages": 1,<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"totalElements": 1,<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"last": false<br>
&nbsp;&nbsp;&nbsp;&nbsp;},<br>
&nbsp;&nbsp;&nbsp;&nbsp;"message": "Users data found for the given gender in this page 1"<br>
}
</details> 
...
<details> 
  <summary>View screenshot</summary>  <br>  
    <img src="docs/screenshots/api-30-1.JPG" width="550" alt="Management Passenger Stats info View Error">
</details>  

**Invalid/Malformed Date Format**  
<details> 
  <summary>View screenshot</summary>  <br>  
    <img src="docs/screenshots/api-30-3.JPG" width="550" alt="Management Passenger Stats info View Error">
</details>  
...
| Field      | Fixed Time | Purpose                |
|------------|------------|------------------------|
| `startDate`  | 00:00:00   | Start of day boundary  |
| `endDate`    | 23:59:59   | End of day boundary    |  
...
#### 📝 Description  
The **Management User Registration API** provides a secure mechanism for creating new **Management-level accounts (ADMIN role)** within **_BookMyRide_**. Access is restricted to existing `ADMIN` users, enforcing strict privilege boundaries and protecting the management layer’s integrity.
 ...
 **Request Body:**  
{<br>
&nbsp;&nbsp;&nbsp; "fullName": "Peter Parker",<br>
&nbsp;&nbsp;&nbsp; "gender": "Male",<br>
&nbsp;&nbsp;&nbsp; "email": "peterparker@gmail.com",<br>
&nbsp;&nbsp;&nbsp; "mobile": "9XXXXXXXXX",<br>
&nbsp;&nbsp;&nbsp; "password": "StrongSecurePassword@123"<br>
}  
...
&nbsp;&nbsp;&nbsp; "fullName": "Peter parker",<br>
&nbsp;&nbsp;&nbsp; "gender": "Male",<br>
&nbsp;&nbsp;&nbsp;"email": "peterparker@gmailcom",<br>
&nbsp;&nbsp;&nbsp;"mobile": "5XXXXXXXXXXX",<br>
&nbsp;&nbsp;&nbsp;"password": "StrongSecurePassword@123"<br>
}  
...
&nbsp;&nbsp;&nbsp; "code": "VALIDATION_FAILED",<br>
&nbsp;&nbsp;&nbsp; "error": [<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; "mobile: Invalid length or starting digit. Expected a 10-digit number starting from 6–9.",<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; "fullName: Invalid format. All name segments must start with an uppercase letter followed by lowercase letters.",<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; "email: Invalid domain format. Accepted domains: gmail.com, yahoo.com."<br>
&nbsp;&nbsp;&nbsp; ],<br>
&nbsp;&nbsp;&nbsp; "status": 400,<br>
&nbsp;&nbsp;&nbsp; "timestamp": "2025-12-27T13:43:08.9686337"<br>
}
</details>  
...
<details> 
  <summary>View screenshot</summary>  <br>  
    <img src="docs/screenshots/api-31-2.JPG" width="550" alt="Management Signup Error">
</details> 
...
**2. Race Condition & Atomic Creation — Current Limitation**  
...
**3. First Admin Account Bootstrapping (ManagementBootstrap)**  
...
Upon the first execution, if the Management table is empty, the console will log a confirmation similar to: `System generated & added a **new Management user** successfully. Ready to log in.`This indicates that the **initial ADMIN account** has been created automatically by the `ManagementBootstrap` component, and the application is ready for use.   
...
**My Personal Motto:**    
> _“**BookMyRide** wasn’t born from a motive to impress…It was born from a decision to rise.”_   
...
Instead of storing random text values, I designed _**BookMyRide**_ using a **strongly typed Enum ecosystem**, which drastically reduced bugs and improved request/response consistency. Enums include: `AcType, BookingStatus, BusType, Country, State, Gender, PermitStatus, PaymentMethod, PaymentStatus, SeatType, Role, ResponseStatus, Code`      
...
This 3-layer system provided several real advantages in **_BookMyRide_**:  
...
**This mechanism offers several advantages:** It centralizes identity creation, improves security by avoiding predictable usernames, eliminates reliance on mobile numbers for internal accounts, and establishes a clear separation between customer and authority login behaviors.   
...
One of the most impactful architectural decisions in **_BookMyRide_** was transitioning from returning raw entity objects to using a **strict DTO-based response model**. This shift brought the platform much closer to real-world API standards. By adopting DTOs, I was able to:   
...
My experience working closely with frontend development strongly influenced how I structured API responses. Drawing inspiration from the YouTube API, I realized the power of clean, nested, and modular JSON structures. Applying this philosophy in **_BookMyRide_**, I ensured that:    
...
One of the architectural improvements I’m most proud of is the redesign of **_BookMyRide_**’s location management system. This upgrade transformed a simple, error-prone implementation into a **scalable, high-performance geographic data system**.  
...
This organization makes **_BookMyRide_** maintainable, readable, scalable, and professional — the exact structure used in enterprise Spring Boot applications.   
...
In my system, the `Bus`, `Booking`, and `AppUser` entities were connected using **bi-directional JPA relationships**. Booking acted as the owning side, referencing both `Bus` and `AppUser`, while each Bus and User maintained a list of Bookings. When invoking `GET /bus` with `findAll()`, every Bus was returned along with its Bookings—but each Booking again referenced Bus and User, which in turn contained their own Booking lists. This circular object graph caused **Jackson** to fall into infinite recursion, ultimately resulting in a **StackOverflowError**.  
...
So I implemented a small but effective normalization step before mapping input to Enum: `input = input.replaceAll("[_ ]", "_");`. This ensured that inputs like `"in put"`, `"in_put"`, or `"INPUT"` all mapped correctly, enabling case-insensitive and consistent Enum handling.   
...
Lesson learned: Even if you don’t directly include a library, **transitive dependencies can introduce vulnerabilities**. Always check the dependency tree and CVE reports when adding major frameworks.
...
## Author / Credits   
**Karthik R – Full-Stack Java Developer**   

[![LinkedIn](https://img.shields.io/badge/linkedin-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/karthik2k/) 
[![GitHub](https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Karthikr32) 
[![Portfolio](https://img.shields.io/badge/portfolio-000000?style=for-the-badge&logo=firefox&logoColor=white)](https://myportfolio-sandy-three-92.vercel.app/) [![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:tkarthik2k@gmail.com)
